...
public:
	...
	uint32_t getManaPercentGainTicks() const {
		return gainManaPercentTicks / g_configManager().getFloat(RATE_MANA_REGEN_SPEED, __FUNCTION__);
	}
	...
	uint32_t getHealthPercentGainTicks() const {
		return gainHealthPercentTicks / g_configManager().getFloat(RATE_MANA_REGEN_SPEED, __FUNCTION__);
	}
...
private:
	...
	uint32_t gainHealthPercentTicks = 6;
