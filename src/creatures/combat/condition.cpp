...
bool ConditionRegeneration::executeCondition(std::shared_ptr<Creature> creature, int32_t interval) {
	...
	if (internalManaPercentTicks >= getManaPercentTicks(creature)) {
			internalManaPercentTicks = 0;
			creature->changeMana((player->getMaxMana() / 100) * manaPercentGain);
	}

	if (internalHealthPercentTicks >= getHealthPercentTicks(creature)) {
			internalHealthPercentTicks = 0;
			creature->changeHealth((player->getMaxHealth() / 100) * healthPercentGain);
	}
...
uint32_t ConditionRegeneration::getManaPercentTicks(std::shared_ptr<Creature> creature) const {
	std::shared_ptr<Player> player = creature->getPlayer();

	if (player != nullptr && isBuff) {
		return manaPercentTicks / g_configManager().getFloat(RATE_SPELL_COOLDOWN, __FUNCTION__);
	}

	return manaPercentTicks;
}

uint32_t ConditionRegeneration::getHealthPercentTicks(std::shared_ptr<Creature> creature) const {
	std::shared_ptr<Player> player = creature->getPlayer();

	if (player != nullptr && isBuff) {
		return healthPercentTicks / g_configManager().getFloat(RATE_SPELL_COOLDOWN, __FUNCTION__);
	}

	return healthPercentTicks;
}
...
bool ConditionDamage::doDamage(std::shared_ptr<Creature> creature, int32_t healthChange) {
	...
	if (field && creature->getPlayer() && attacker && attacker->getPlayer()) {
		damage.primary.value = static_cast<int32_t>(std::round(damage.primary.value));
	}
