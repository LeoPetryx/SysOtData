...
uint32_t MoveEvent::EquipItem(const std::shared_ptr<MoveEvent> moveEvent, std::shared_ptr<Player> player, std::shared_ptr<Item> item, Slots_t slot, bool isCheck) {
	...
	if (it.abilities) {
		...
		if (it.abilities->regeneration) {
			...
			if (it.abilities->getHealthPercentGain() != 0) {
				condition->setParam(CONDITION_PARAM_HEALTHPERCENTGAIN, it.abilities->getHealthPercentGain());
			}

			if (it.abilities->getHealthPercentTicks() != 0) {
				condition->setParam(CONDITION_PARAM_HEALTHPERCENTTICKS, it.abilities->getHealthPercentTicks());
			}

			if (it.abilities->getManaPercentGain() != 0) {
				condition->setParam(CONDITION_PARAM_MANAPERCENTGAIN, it.abilities->getManaPercentGain());
			}

			if (it.abilities->getManaPercentTicks() != 0) {
				condition->setParam(CONDITION_PARAM_MANAPERCENTTICKS, it.abilities->getManaPercentTicks());
			}
