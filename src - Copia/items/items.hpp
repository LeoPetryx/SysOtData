...
public:
	...
	uint32_t getManaPercentGain() const {
		return manaPercentGain * g_configManager().getFloat(RATE_MANA_REGEN, __FUNCTION__);
	}


	void setManaPercentGain(uint32_t value) {
	...
	void setManaPercentTicks(uint32_t value) {
	...
	uint32_t getHealthPercentGain() const {
		return healthPercentGain ;
	}


	void setHealthPercentGain(uint32_t value) {
		healthPercentGain = value;
	}

	void setHealthPercentTicks(uint32_t value) {
		healthPercentTicks = value;
	}

	uint32_t getHealthPercentTicks() const {
...
private:
	...
	uint32_t manaPercentTicks = 0;
	uint32_t healthPercentGain = 0;
	uint32_t healthPercentTicks = 0;
};
