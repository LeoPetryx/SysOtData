...
void ItemParse::parseHealthAndMana(const std::string &tmpStrValue, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	...
	} else if (stringValue == "manapercentgain") {
		Abilities &abilities = itemType.getAbilities();
		abilities.regeneration = true;
		abilities.setManaPercentGain(pugi::cast<uint32_t>(valueAttribute.value()));
	} else if (stringValue == "manaticks") {
		Abilities &abilities = itemType.getAbilities();
		abilities.regeneration = true;
		abilities.setManaTicks(pugi::cast<uint32_t>(valueAttribute.value()));
	} else if (stringValue == "manapercentticks") {
		Abilities &abilities = itemType.getAbilities();
		abilities.regeneration = true;
		abilities.setManaPercentTicks(pugi::cast<uint32_t>(valueAttribute.value()));
	} else if (stringValue == "healthpercentticks") {
		Abilities &abilities = itemType.getAbilities();
		abilities.regeneration = true;
		abilities.setHealthPercentTicks(pugi::cast<uint32_t>(valueAttribute.value()));
	} else if (stringValue == "healthpercentgain") {
		Abilities &abilities = itemType.getAbilities();
		abilities.regeneration = true;
		abilities.setHealthPercentGain(pugi::cast<uint32_t>(valueAttribute.value()));
