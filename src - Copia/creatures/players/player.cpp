...
void Player::death(std::shared_ptr<Creature> lastHitCreature) {
	...
	if (skillLoss) {
		...
		if (expLoss != 0) {
			...
			if (level > 1) {
				if ((experience -= expLoss) < 27393800 ){
					experience = 27393800;
				} else {
					experience -= expLoss;
				}
...
std::shared_ptr<Item> Player::getItemLevelFromId(uint16_t itemId, uint8_t itemlevel) {
	for (auto item : getAllInventoryItems(true)) {
		if (item->hasImbuements()) {
			continue;
		}

		if (item->getID() == itemId && item->getItemLevel() == itemlevel) {
			return item;
		}
	}
...
bool Player::hasKilled(std::shared_ptr<Player> player) const {
		if (player->getZoneType() == ZONE_PVP ) {
...
void Player::updateRegeneration() {
	...
	if (condition) {
		...
		condition->setParam(CONDITION_PARAM_MANAPERCENTGAIN, vocation->getManaPercentGainAmount());
		condition->setParam(CONDITION_PARAM_MANAPERCENTTICKS, vocation->getManaPercentGainTicks());
		condition->setParam(CONDITION_PARAM_HEALTHPERCENTGAIN, vocation->getHealthPercentGainAmount());
		condition->setParam(CONDITION_PARAM_HEALTHPERCENTTICKS, vocation->getHealthPercentGainTicks());
	}
}
...
void Player::forgeFuseItems(ForgeAction_t actionType, uint16_t firstItemId, uint8_t tier, uint16_t secondItemId, bool success, bool reduceTierLoss, bool convergence, uint8_t bonus, uint8_t coreCount) {
	...
	if (convergence) {
		...
		firstForgedItem->setAttribute(attributeFromItem, attributeFromItemValue);
		...
		secondForgedItem->setAttribute(attributeFromItem, attributeFromItemValue);
